...
int add_animal(int animal_id, std::string animal_type, std::string animal_name, int animal_age)
{
    int total_animals = 0;
    ...
    {
        if (deleted_ids.empty()) { animal_id = static_cast<int>(animals.size()) + 1; }
        else { animal_id = deleted_ids.front(); deleted_ids.pop(); }
    }
...
void debug_screen()
{
    ...
    {
        for (auto const& x : animal_groups) { std::cout << x.first << "s = " << x.second << "\n"; }
        std::cout << "\n";
    }
    ...
    if (deleted_ids.size() > 0)
    {
        ...
        for (int i = 0; i < deleted_ids.size() - 1; i++)
        {
...
{
    if (animals.empty()) { std::cout << "\nNo animals currently in the system.\n\n"; }
    else {
        if (animal_groups.size() != 1) { std::cout << "\nThere are " <<
            animal_groups.size() << " animal types in the system:\n\n"; }
        ...
        for (auto const& x : animal_groups)
        {
            ...
            for (auto& tuple : animals)
            {
                ...
                {
                    std::cout << std::get<2>(tuple) << "\tAge " << std::get<3>(tuple)
                        << "\t ID #" << std::get<0>(tuple) << "\n";
                }
            }
...
void input_animal()
{
    ...
    std::cin >> type_input;
    std::cin.ignore(1, '\n');   // flushes the newline from the previous cin for the next getline

    for (char& c : type_input) { c = std::tolower(c); } // all characters switched to lower case

    std::cout << "Choose a name for your " << type_input << ": ";
    std::getline(std::cin, name_input); // getline used instead as the name may contain spaces
    ...
    // append the new data to data_log.csv, so it is saved after the program is terminated

    std::ofstream data_log;
    data_log.open("data.csv", std::ios_base::app);  // append from end instead of overwriting
    data_log << animal_id << "," << type_input << "," << name_input << "," << age_input << "\n";
    data_log.close();
    ...
    while (deleted_ids_log.peek() != EOF)
    {
        ...
        if (animal_id != stoi(deleted_id)) { temp_log << animal_id << "\n"; }
    }
...
void load_data()
{
    ...
    if (!data_log.is_open()) std::cout << "Error: data_log.csv open." << "\n";
    ...
    while (data_log.peek() != EOF)
    {
        ...
        if (!animal_id.empty()) { add_animal(stoi(animal_id), animal_type, animal_name,
    ...
    while (deleted_ids_log.peek() != EOF)
    {
        ...
        if (!animal_id.empty()) { deleted_ids.push(stoi(animal_id)); }
    }
...
void remove_animal()
{
    ...
    for (auto& tuple : animals)
    {
        ...
        {
            std::map<std::string, int>::iterator it = animal_groups.find(std::get<1>(tuple));

            if (it != animal_groups.end()) { it->second -= 1; }
            ...
            // rewrite data to new temp.csv, so rather than sorting and finding the id to remove,
            // the id won't be included in the new file with an if statement done 11 lines down.
            ...
            remove("data.csv");
            if (rename("temp.csv", "data.csv") != 0) { std::cout <<
                "Error: temp.csv is open." << "\n"; }
...
{    
    ...
    while (true)
    {
        ...
        for (char& c : command_input) { c = std::tolower(c); } // characters switched to lower case
        
        // switch statement would be more efficient if the user was asked to input numbers instead
        // of strings, but the code that accepts strings is less efficient than if else statements
        
        if      (command_input == "/add")       { input_animal();       continue; }
        else if (command_input == "/debug")     { debug_screen();       continue; }
        else if (command_input == "/display")   { display_animals();    continue; }
        else if (command_input == "/end")       {                       break;    }
        else if (command_input == "/help")      { help_options();       continue; }
        else if (command_input == "/remove")    { remove_animal();      continue; }
